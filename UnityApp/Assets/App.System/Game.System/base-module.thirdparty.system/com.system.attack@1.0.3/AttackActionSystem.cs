...
namespace ECSGame.AttackModule
{
    ...
    public class AttackActionSystem : AEntitySystem<AttackAction>
    {
        ...
        public static bool TryStartAttack(EcsEntity actor, long targetId, out AttackFailureReason fail, out long newActionId)
        {
            // 基础校验
            if (targetId <= 0)
            {
            ...
            actor.Dispatch<IAttackConfigProvider>(p => cfg = p.GetConfig(actor, 0));
            if (cfg == null)
            {
