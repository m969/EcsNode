...
namespace ECSGame
{
        ...
        IDisable<EcsEntity, AIComponent>
    {
        ...
        public static void StopBehaviour(EcsEntity entity, long behaviourId)
        {
            var component = entity.GetComponent<AIComponent>();
            if (component.Behaviour2NodeQueue.TryGetValue(behaviourId, out var queue))
            {
                queue.Clear();
            }
            if (component.AIBehaviours.ContainsKey(behaviourId))
            {
                component.AIBehaviours.Remove(behaviourId);
            }
        }
